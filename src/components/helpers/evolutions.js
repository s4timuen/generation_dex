...
function getEvolutionChain(context, dataFilter) { 
    ...
    context.$store.getters.pokedex.resource(context.$store.getters.pokemonData.species.url)
    .then(async function(response) { 

        // get first of evolution chain
        return await context.$store.getters.pokedex.resource(response.evolution_chain.url)
     ...
     .then(async function(response) {
        ...
        for(let index = 0; index < 3; index++) { // for all available evolutions
